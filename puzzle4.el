...
(defvar *rooms* '("hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]"
...
(defun extract-components (str)
  ...
  (let ((name (match-string 1 str))
	(id (match-string 2 str))
	(checksum (match-string 3 str)))
    (list name id checksum)))
...
(defun hash-table-top-n-values (table n)
  ...
  (subseq (cl-sort (hash-table-alist table)
		   (lambda (x y)
		     (cond ((> (cdr x) (cdr y)) t)
			   ((< (cdr x) (cdr y)) nil)
			   (;; (= (cdr x) (cdr y))
...
(defun top-5-chars (str)
  (cl-loop
   with hist = (make-hash-table)
   for char across str
   when (<= ?a char ?z)
   do (incf (gethash char hist 0))
   finally (return (hash-table-top-n-values hist 5))))

(defun get-id-if-real (str)
  (multiple-value-bind (name id checksum)
      (extract-components str)
    (if (string-equal checksum
		      (apply #'string
			     (mapcar #'car (top-5-chars name))))
	(string-to-number id)
      0)))

(defun solve-puzzle (&optional rooms)
  (reduce #'+ (mapcar #'get-id-if-real (or rooms *rooms*))))

(defun test-solve-puzzle ()
  (solve-puzzle '("aaaaa-bbb-z-y-x-123[abxyz]"
...
(defun rotate-char (c n)
  (if (= c ?-)
      ? 
    (let* ((char-delta (- c ?a))
	   (rot-delta (mod (+ char-delta n) (- ?z ?a -1))))
      (+ ?a rot-delta))))

(defun room-real-p (str)
  (multiple-value-bind (name id checksum)
      (extract-components str)
...
(defun solve-puzzle-2 (&optional rooms)
  (cl-loop
   with decoded = nil
   for room in (or rooms *rooms*)
   do (multiple-value-bind (name id checksum)
	  (extract-components room)
	(when (string-equal checksum
			    (apply #'string
				   (mapcar #'car (top-5-chars name))))
	  (push
	   (cons
	   (apply #'string
		  (mapcar (lambda (x)
			    (rotate-char x (string-to-number id)))
			  name))
	   id) decoded)))
