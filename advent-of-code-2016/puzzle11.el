...
;; You come upon a column of four floors that have been entirely
;; sealed off from the rest of the building except for a small
;; dedicated lobby. There are some radiation warnings and a big sign
;; which reads "Radioisotope Testing Facility".

;; According to the project status board, this facility is currently
;; being used to experiment with Radioisotope Thermoelectric
;; Generators (RTGs, or simply "generators") that are designed to be
;; paired with specially-constructed microchips. Basically, an RTG is
;; a highly radioactive rock that generates electricity through heat.
...
;; generate an elecromagnetic radiation shield when
;; powered. Unfortunately, they can only be powered by their
;; corresponding RTG. An RTG powering a microchip is still dangerous
;; to other microchips.
...
;; measure, the elevator will only function if it contains at least
;; one RTG or microchip. The elevator always stops on each floor to
...
;; When you enter the containment area, you and the elevator will
...
;; Hydrogen, L for Lithium, M for Microchip, and G for Generator), the
...
;;     be temporarily connected to their corresponding generators
;;     while the elevator recharges, preventing either of them from
;;     being fried:
...
;;     is on the destination floor, you can connect
...
;; In this arrangement, it takes 11 steps to collect all of the
;; objects at the fourth floor for assembly. (Each elevator stop
;; counts as one step, even if nothing is added to or removed from
;; it.)
