...
(defvar *test-instructions* '("value 5 goes to bot 2"
			      "bot 2 gives low to bot 1 and high to bot 0"
			      "value 3 goes to bot 1"
			      "bot 1 gives low to output 1 and high to bot 0"
			      "bot 0 gives low to output 2 and high to output 0"
...
(defvar *instructions* '("bot 123 gives low to bot 191 and high to bot 162"
...
(defvar *factory* (list :bots nil
			:outputs nil
			:queue (copy-list *instructions*)))

(defvar *test-factory* (list :bots nil
			     :outputs nil
			     :queue (copy-list *test-instructions*)))

(defun parse (instruction &optional factory)
  (let ((factory (or factory *factory*)))
    (cond
     ((string-match "bot \\([0-9]+\\) gives low to \\(\\(\\w+\\) \\([0-9]+\\)\\) and high to \\(\\(\\w+\\) \\([0-9]+\\)\\)" instruction)
      (let ((bot-actor (string-to-number (match-string 1 instruction)))
	    (low-target-type (match-string 3 instruction))
	    (low-target-number (string-to-number (match-string 4 instruction)))
	    (high-target-type (match-string 6 instruction))
	    (high-target-number (string-to-number (match-string 7 instruction))))
	(bot-act factory bot-actor
		 low-target-type low-target-number
		 high-target-type high-target-number)))
     ((string-match "value \\([0-9]+\\) goes to bot \\([0-9]+\\)" instruction)
      (let ((value (string-to-number (match-string 1 instruction)))
	    (target-bot (string-to-number (match-string 2 instruction))))
	(assign-value factory value target-bot)))
     (t
      (error "Instruction not recognized: %s" instruction)))))

(defun bot-act (factory actor
			low-type low-target
			high-type high-target)
  (let ((actor-number-list (copy-list (cl-getf (cl-getf factory :bots) actor))))
    (if actor-number-list
	(when (= (length actor-number-list) 2)
	  ;; (when (null (set-difference actor-number-list (list 17 61)))
	  ;;   (print "Actor: %s" actor))
	  (map 'list (lambda (type target what)
		       (if (string-equal type "bot")
			   (push what (cl-getf (cl-getf factory :bots) target))
			 (push what (cl-getf (cl-getf factory :outputs) target))))
	       (list low-type high-type)
	       (list low-target high-target)
	       (sort actor-number-list #'<))
	  (setf (cl-getf (cl-getf factory :bots) actor) nil)
	  t)
      (setf (cl-getf (cl-getf factory :bots) actor) nil))))

(defun assign-value (factory value target-bot)
  (push value (cl-getf (cl-getf factory :bots) target-bot)))

(defun solve-puzzle (&optional factory)
  (let ((factory (or factory *factory*)))
    (cl-loop
     while (cl-getf factory :queue)
     do (cl-loop
	 for instr on (cl-getf factory :queue)
	 until (parse (car instr) factory)
	 finally (progn (rplaca instr (cadr instr))
			(rplacd instr (cddr instr)))))))
