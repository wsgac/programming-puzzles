...
(defun day-04-parse-input (input)
  "Parse passport entries and convert them into a list of
alists (with CARs being the property names and CDRs their
values)."
  (mapcar (lambda (item)
	    (mapcar (lambda (pair)
		      (apply #'cons (split-string pair ":")))
		    (split-string item "[\n ]")))
	  (split-string input "\n\n")))

(defun validate-entry (entry)
  "Check if ENTRY represents a valid passport entry by checking
  if it contains all the required fields."
  (null (set-difference *day-04-required-entries* (mapcar #'car entry) :test #'equal)))

(defun advanced-validate-entry (entry)
  "Validate ENTRY according to the extended specification."
  (labels ((validate-byr (x)
		(<= 1920 (string-to-number x) 2002))
	   (validate-iyr (x)
		(<= 2010 (string-to-number x) 2020))
	   (validate-eyr (x)
		(<= 2020 (string-to-number x) 2030))
	   (validate-hgt (x)
		(cond
		 ((string-match "\\([0-9]+\\)cm" x)
		  (<= 150 (string-to-number (match-string 1 x)) 193))
		 ((string-match "\\([0-9]+\\)in" x)
		  (<= 59 (string-to-number (match-string 1 x)) 76))))
	   (validate-hcl (x)
			 (string-match "#[0-9a-f]\\{6\\}" x))
	   (validate-ecl (x)
		(member x '("amb" "blu" "brn" "gry" "grn" "hzl" "oth")))
	   (validate-pid (x)
			 (string-match "^[0-9]\\{9\\}$" x)))
    (let ((byr (assoc "byr" entry))
	  (iyr (assoc "iyr" entry))
	  (eyr (assoc "eyr" entry))
	  (hgt (assoc "hgt" entry))
	  (hcl (assoc "hcl" entry))
      ...
      (and byr iyr eyr hgt hcl ecl pid
	   (validate-byr (cdr byr))
	   (validate-iyr (cdr iyr))
	   (validate-eyr (cdr eyr))
	   (validate-hgt (cdr hgt))
	   (validate-hcl (cdr hcl))
	   (validate-ecl (cdr ecl))
	   (validate-pid (cdr pid))))))

(defun day-04-problem-1 (input)
  "Count valid passport entries in INPUT."
  (reduce (lambda (acc el)
	    (+ acc (if (validate-entry el) 1 0)))
	  (day-04-parse-input input)
	  :initial-value 0))

(defun day-04-problem-2 (input)
  "Count valid passport entries in INPUT."
  (reduce (lambda (acc el)
	    (+ acc (if (advanced-validate-entry el) 1 0)))
	  (day-04-parse-input input)
	  :initial-value 0))

(defvar *day-04-required-entries*
  '("byr" "iyr" "eyr" "hgt" "hcl" "ecl" "pid"))

(defvar *day-04-test-valid-input*
...
(defvar *day-04-test-invalid-input*
  "eyr:1972 cid:100
