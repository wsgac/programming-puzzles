...
(defun day-06-get-unique-from-group (input)
  (loop
   with h = (make-hash-table)
   for c across (string-join (split-string input))
   do (setf (gethash c h) nil)
   finally (return (hash-table-keys h))))

(defun day-06-count-unanimous-from-group (input)
  (loop
   with p = (length (split-string input))
   with h = (make-hash-table)
   for c across (string-join (split-string input))
   do (incf (gethash c h 0))
   finally (return (count-if (lambda (x) (= x p)) (hash-table-values h)))))

(defun day-06-problem-1 (input)
  (loop
   for group in (split-string input "\n\n")
   sum (length (day-06-get-unique-from-group group))))

(defun day-06-problem-2 (input)
  (loop
   for group in (split-string input "\n\n")
   sum (day-06-count-unanimous-from-group group)))
