...
(defun split-ip (ip)
  (cl-loop
   for part in (split-string ip "[][]")
   for i from 0
   if (evenp i)
   collect part into free
   else
   collect part into bracketed
   finally (return (list :free free
                         :bracketed bracketed))))

(defun detect-abba (str)
  (and
...
(defun supports-tls-p (ip)
  (let ((split-ip (split-ip ip)))
    (and (some #'detect-abba (getf split-ip :free))
         (not (some #'detect-abba (getf split-ip :bracketed))))))

(defun solve-puzzle (&optional ips)
  (cl-loop
   for ip in (or ips *ip-addresses*)
   do
   (when (supports-tls-p ip)
     (print ip))
   count (supports-tls-p ip)))
