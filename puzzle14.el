...
;; the MD5 of a pre-arranged salt (your puzzle input) and an
;; increasing integer index (starting with 0, and represented in
;; decimal); the resulting MD5 hash should be represented as a string
...
;; Considering future
;; hashes for five-of-a-kind sequences does not cause those hashes to
;; be skipped; instead, regardless of whether the current hash is a
;; key, always resume testing for keys starting with the very next
;; hash.
...
;; 64th key.  So, using our example salt of abc, index 22728 produces
;; the 64th key.

;; Given the actual salt in your puzzle input, what index produces
;; your 64th one-time pad key?

(defvar *salt* "qzyelonm")

(defun md5-pad-p (sum next-1000)
  (when (string-match "\\([0-9a-f]\\)\\1\\{2,\\}" sum)
    (let ((letter (match-string 1 sum)))
      (some (lambda (s)
	      (string-match
	       (format "%s\\{5,\\}" letter) s))
	    next-1000))))

(defun solve-puzzle (&optional salt)
  (cl-loop
   with salt = (or salt *salt*)
   with keys = nil
   with sums = (cl-loop
		for i from 1 to 1000
		collect (md5 (format "%s%d" salt i)))
   with current = (md5 (format "%s0" salt))
   with last = 1000
   with lastcons = (last sums)
   until (= (length keys) 64)
   when (md5-pad-p current sums)
   collect current into keys
   do
   (incf last)
   (setf current (car sums))
   (setf sums (cdr sums))
   (rplacd lastcons (cons (md5 (format "%s%d" salt last)) nil))
   (setf lastcons (cdr lastcons))
   finally (return (values keys (- last 1001)))))
...
(defun md5-stretch (string)
  (cl-loop
   for result = (md5 string) then (md5 result)
   repeat 2016
...
(defun solve-puzzle (&optional salt)
  (cl-loop
   ...
   with sums = (cl-loop
		for i from 1 to 1000
		collect (md5-stretch (format "%s%d" salt i)))
   with current = (md5-stretch (format "%s0" salt))
   with last = 1000
   with lastcons = (last sums)
   until (= (length keys) 64)
   when (md5-pad-p current sums)
   ...
   (setf current (car sums))
   (setf sums (cdr sums))
   (rplacd lastcons (cons (md5-stretch (format "%s%d" salt last)) nil))
   (setf lastcons (cdr lastcons))
   finally (return (values keys (- last 1001)))))
